...
def fade_one_segment(count):
    count=count&511
    if count < 256:
        send_byte_TrainLED(Pin(TrainLED_PIN, Pin.OUT), count)
    else:
        send_byte_TrainLED(Pin(TrainLED_PIN, Pin.OUT), 512-count)
...
while True:
    ...
    # Fade the three segments with a phase shift in between
    fade_one_segment(counter)
    fade_one_segment(counter+128)
    fade_one_segment(counter+256)
    utime.sleep_ms(50)